## srun -p mllm-align --quotatype=reserved --gres=gpu:1 --cpus-per-task=4 --time=300 python llama_prompts.py 
...
def generate_llama_choice_negative_prompt_data(
    dataset, file_name="llama_choice_negative_prompt_data.json", starting_idx=0
):
    responses, articles, keys = load_data(dataset)
    ...
    for key in keys[starting_idx:]:
        ...
        for other in ["human", "claude", "gpt4", "gpt35"]:
            ...
            question["forward_comparison_prompt"] = LLAMA_PROMPT_TEMPLATE.format(
                system_prompt=COMPARISON_SYSTEM_PROMPT,
                user_prompt=COMPARISON_PROMPT_TEMPLATE_WITH_WORSE.format(
                    article=article, summary1=self_response, summary2=other_response
                ),
            )
            question["backward_comparison_prompt"] = LLAMA_PROMPT_TEMPLATE.format(
...
#
generate_llama_choice_prompt_data("cnn")
generate_llama_choice_negative_prompt_data("cnn")
